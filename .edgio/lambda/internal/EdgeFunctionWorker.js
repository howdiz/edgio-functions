...
var EdgeFunctionMetrics = class {
  constructor() {
    ...
    this.WASM_METRIC_JSON_FIRST_INDEX = 1e3;
...
var WasmProgram = class {
  ...
  getStringFromGuestMemory(memory, offset, len) {
    if (len > MAX_GUEST_STRING_BYTES) {
      return new WasmError(5 /* invalid_argument */, `(length ${len} > ${MAX_GUEST_STRING_BYTES} bytes) Argument is too long`);
    }
...
var EdgeFunctionRequest = class {
  ...
  static validate_uri(uri) {
    ...
    if (uri.length > MAX_HEADER_BYTES2) {
      return new WasmError(8 /* limit_exceeded */, `URI length is too long (length ${uri.length} > ${MAX_HEADER_BYTES2})`);
    }
...
var HTTP_HEADER_DELIMETER_BYTES = 4;
function isHttpVarNameSpace(ns) {
  return ns === ENV_VAR_NAMESPACE_EDGIO_GEO || ns === ENV_VAR_NAMESPACE_EDGIO_VIRT || ns === ENV_VAR_NAMESPACE_EDGIO_WURFL;
}
    ...
    return {
      env: {
        ...
        hostcall_req_set_header: (id, key_offset, key_length, val_offset, val_length) => {
          ...
          if (request.headers.get_total_bytes() + key_length + val_length + HTTP_HEADER_DELIMETER_BYTES > MAX_HEADER_BYTES) {
            return handleWasmError({
        ...
        hostcall_resp_set_header: (id, key_offset, key_length, val_offset, val_length) => {
          ...
          if (response.headers.get_total_bytes() + key_length + val_length + HTTP_HEADER_DELIMETER_BYTES > MAX_HEADER_BYTES) {
            return handleWasmError({
              status: 8 /* limit_exceeded */,
